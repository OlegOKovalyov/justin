...
error_log('$this->sign');error_log($this->sign);
        $this->apikey = get_option('morkvajustin_apikey');
    ...
    */
    ...
    public function getWarehouse($countryCode, $descr)
    {
        $filterarray = array(
          "name" => "descr",
          "comparison" => "equal",
        ...
        $jsongetwarehouses = array(
          ...
          "TOP" => 50,
          "params" =>array('language' => $countryCode)
        );
        if (null != $descr) {
...
error_log('$remote_post');error_log(print_r($remote_post,1));
        return $remote_post['body'];
    }

    public function getWarehouses($countryCode, $regionId = null)
    {
        $filterarray = array(
          "name" => "region",
          "comparison" => "equal",
