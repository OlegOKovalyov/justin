...
class MJS_Plugin_Loader
{
     ...
     * @since 1.0.0
     */
    public function mrkvjustin_add_meta_boxes()
    {
    ...
    public function woo_column_get_data($column)
    {
        ...
        if ($column == 'created_invoice') {
            ...
            if ( ! empty( $results ) || !empty( $meta_ttn ) ) {
                ...
                $img = 'justin-gray.png'; // gray Justin logo
            }
    ...
    {
        global $wpdb;
        $invoices_table_name = $wpdb->prefix . 'justin_ttn_invoices';
        ...
        if ( empty( $meta_ttn ) ) {
            global $wpdb;
            $result = $wpdb->get_results( "SELECT * FROM {$invoices_table_name} WHERE order_id = '$order_id'", ARRAY_A );
        ...
        if ( ! empty( $meta_ttn ) ) {
            ...
            echo '<p><a  class="mrkvjs_stickerprint button" alert="У новій вкладці відкриється документ для друку"
                target="_blank" href="https://api.justin.ua/justin_pms/hs/api/v1/printSticker/order?order_number=' .
                    $order_id . '&api_key=' . get_option( 'morkvajustin_apikey' ) . '">' .
                ' <img src="' . plugins_url('/barcode.png', __FILE__) . '" style="vertical-align:text-bottom;margin-right:5px;" /> Друк стікера </a></p>';
        } else {
